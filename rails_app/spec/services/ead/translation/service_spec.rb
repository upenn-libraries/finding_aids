...
XML
        ...
        it 'applies underline markup and class accordingly' do
          expect(node).to have_xpath('//span[@class="underline"]', count: 1, text: 'Underline')
        end

        it 'applies sub- and super-script markup accordingly' do
          expect(node).to have_xpath '//sup', count: 1, text: 'Superscript'
          expect(node).to have_xpath '//sub', count: 1, text: 'Subscript'
        end

        it 'applies markup for bolded text' do
          expect(node).to have_xpath '//strong', count: 1, text: 'Bolded'
        end

        it 'applies markup for italicized text' do
          expect(node).to have_xpath '//em', count: 2, text: /Italicized/
        end

        it 'applies markup for small-caps text' do
          expect(node).to have_xpath '//span[@class="small-caps"]', count: 1, text: 'Small Caps'
        end

        it 'wraps text with quotations accordingly' do
          expect(node).to have_xpath '//span', count: 1, text: '"Double Quotes"'
          expect(node).to have_xpath '//span', count: 1, text: "'Single Quotes'"
        end
      end
...
XML
        ...
        it 'applies combined bold and italic markup accordingly' do
          expect(node).to have_xpath '//strong/em', count: 1, text: 'Bolded and Italicized'
        end

        it 'applies combined bold and small-caps markup' do
          expect(node).to have_xpath '//strong[@class="small-caps"]', count: 1, text: 'Bolded Small Caps'
        end

        it 'wraps text with quotations accordingly' do
          expect(node).to have_xpath '//strong', count: 1, text: '"Bolded with Double Quotes"'
          expect(node).to have_xpath '//strong', count: 1, text: "'Bolded with Single Quotes'"
        end
      end
    end
  end
...
XML
      ...
      it 'converts nodes to proper definition list structure' do
        expect(node).to have_xpath '//dl/dt', count: 2, text: /Label/
        expect(node).to have_xpath '//dl/dd', count: 2, text: /Content/
      end
    end
...
XML
      ...
      it 'converts nodes to proper unordered list structure' do
        expect(node).to have_xpath '//ul/li', count: 3, text: /Item/
      end
    end
...
XML
      ...
      it 'converts nodes to proper ordered list structure' do
        expect(node).to have_xpath '//ol/li', count: 4, text: /Item/
      end
    end
...
XML
      ...
      it 'converts nodes to properly reflect a nested list' do
        expect(node).to have_xpath '//ol/li[1]/dl/dt', count: 1, text: /Label/
        expect(node).to have_xpath '//ol/li', count: 2, text: /Item/
      end
    end
  end
...
XML
    ...
    it 'converts them to HTML <a> tags with href attribute' do
      expect(node).to have_xpath '//a[@href="https://www.extref.old" and @rel="noopener"]',
                                 text: 'This is a link'
    end
  end
end
